...
<@form.form modelAttribute="product" id="photos-resize" enctype="multipart/form-data">
    <#if product.label?? && product.label.originalPhoto?? && (product.label.originalPhoto?length > 0)>
        <div id="crop-image" class="crop-widget mb clearfix">
            <div class="mbh clearfix">
                <div class="large">
                    <img src="${cp}files/${product.label.resizedPhoto}?${timestamp}" width="456"
                         alt="<@spring.message code="crop.originalPhoto"/>" id="origPhoto" class="image"/>
                ...
                <div class="small">
                    <img src="${cp}files/${product.label.resizedPhoto}?${timestamp}"
                         alt="<@spring.message code="crop.croppedPhoto"/>" id="croppedPhoto" class="thumbnail"/>
                </div>
            </div>
        ...
        <script type="text/javascript">
            ...
            lib.ready(function () {
                lib.widget.crop.run(lib.dom.byId("crop-image"), {
                    ...
                    setSelect: [${product.label.cropping.x1}* 456 /${product.label.width},${product.label.cropping.y1}* 456 /${product.label.width},${product.label.cropping.x2}* 456 /${product.label.width},${product.label.cropping.y2}*
                456 /${product.label.width}]
            });
            })
            // ]]>
        </script>
    ...
    <#if product.ingredients?? && product.ingredients.originalPhoto?? && (product.ingredients.originalPhoto?length > 0)>
        <div id="crop-ingredients" class="crop-widget mb clearfix">
            <div class="mbh clearfix">
                <div class="large">
                    <img src="${cp}files/${product.ingredients.resizedPhoto}?${timestamp}"
                         alt="<@spring.message code="createproduct.form.conservantsPicture"/>" id="conservantsPhoto"
                         class="image"/>
                ...
                <div id="crop-editing-ingredients-photo" class="overflow-drag-widget small overflow">
                    <div class="overflow-container">
                        <img src="${cp}files/${product.ingredients.resizedPhoto}?${timestamp}"
                             alt="<@spring.message code="createproduct.form.conservantsPicture"/>" class="thumbnail"/>
                    </div>
                </div>
                <script type="text/javascript">
                    // <![CDATA[
