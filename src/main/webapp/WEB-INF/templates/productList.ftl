...
<#if searchResults?? >
    ...
    <#elseif searchResults.isProduct()>
        ...
        <#assign feedLinks>
        <span id="rss"><a href="products.rss?q=${q}" alt="RSS" title="<@spring.message code="searchResults.rss"/>"><img
                src="../img/rss.png" alt="RSS" title="<@spring.message code="searchResults.rss"/>"/></a> <a
                href="products.atom?q=${q}" alt="ATOM" title="<@spring.message code="searchResults.atom"/>"><img
                src="../img/rss.png" alt="ATOM" title="<@spring.message code="searchResults.atom"/>"/></a> <a
                href="products.JSON?q=${q}" alt="JSON" title="<@spring.message code="searchResults.json"/>"><img
                src="../img/rss.png" alt="JSON" title="<@spring.message code="searchResults.json"/>"/></a></span>
        </#assign>
    <#else>
        <#assign productDescription>
        ...
        <#if criteria.hasFilter()>
            ...
            <#list criteria.filters as filter>
                <#if filter.descriptionArgument != "ApprovedProductFilter">
                    <a href="${cp}spring/productList/remove?filterIndex=${filter_index}"
                       title="<@spring.message code="productList.filter.remove" />"
                       class="ico remove"><span><@spring.message code="${filter.description}" arguments="${filter.descriptionArgument!''}"/></span></a>
                </#if>
            </#list>
        </#if>
        </p>
