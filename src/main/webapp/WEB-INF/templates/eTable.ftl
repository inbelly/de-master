            ...
            <div class="additive clearfix <#if !createProductInProgress?? && !e.approved>notapproved</#if><#if !e_has_next> last</#if>">
                <div class="additive-info-wrapper clearfix">
                    <div class="additive-info-quick">
                            ...
                            <#if product??>
                                <#if editing?? && editing && product.canBeEditedBy(currentUser)>
                                    <p class="action-delete">
                                        <a class="red" href="${cp}spring/product/remove?eid=${e.id}&id=${product.id}"
                                           onclick="return confirm('<@spring.message code="confirm.really" />');"><@spring.message code="eTable.deleteFromProduct"/></a>
                                    </p>
                                </#if>
                            <#else>
                                <#if currentUser?? && currentUser.admin>
                                ...
                                <#-- <#if !e.approved>
                                    ...
                                    <#if 0 == e.productCount >
                                        <p class="action-delete">
                                            <a class="red" href="${cp}spring/e/delete?eid=${e.id}"
                                               onclick="return confirm('<@spring.message code="confirm.really" />');"><@spring.message code="eTable.delete"/></a>
                                        </p>
                                    </#if>
                                    <p class="action-delete">
                                        <a class="red"
                                           href="${cp}spring/eEdit?eid=${e.id}"><@spring.message code="eTable.edit"/></a>
                                    </p>
                        ...
                        <div>
                            <img src="${cp}images/hazard-${e.category}.png" width="45" height="45"
                                 alt="<@spring.message code="${hazardDescriptions[e.category]}" />"
                                 title="<@spring.message code="${hazardDescriptions[e.category]}" />" class="hazard"/>
                        ...
                        -->
                            ...
                            <p class="nmb"><strong><a
                                    href="${cp}spring/productList/filterByE?eid=${e.id}&amp;name=${e.number?url('utf-8')}"><@spring.message code="eTable.detectedInProducts" arguments="${e.approvedProductCount}"/></a></strong>
                        </div>
                    </div>

                    <div class="additive-info-details">
                        <#if (e.diseases?? && e.diseases?length>0)><p class="diseases">
                            <strong><@spring.message code="eTable.diseases"/>: </strong>${e.diseases?xhtml}</p></#if>
                        <#if e.origin??><p>${e.origin?xhtml}</p></#if>
                        <#if (e.linksDiseases?? && e.linksDiseases?length>0)><p>
                            <strong><@spring.message code="eTable.diseasesLinks"/>:</strong>
                            <#list e.linksDiseases?split(' ') as l>
                                <a href="${l}">${l}</a>
                            </#list>
                        </p>
                        </#if>
                        <#if (e.linksBanned?? && e.linksBanned?length>0)><p>
                            <strong><@spring.message code="eTable.bannedLinks"/>:</strong>
                        ...
                        <#if (e.nameEnglish?? && e.nameEnglish?length>0)>
                            <p class="secondary"><a
                                    href="http://www.ncbi.nlm.nih.gov/sites/entrez?cmd=Search&amp;db=PubMed&amp;term=${e.nameEnglish?url('utf-8')}"><@spring.message code="eTable.moreInfo"/></a>
                            </p>
                        </#if>
                    </div>
                </div>
