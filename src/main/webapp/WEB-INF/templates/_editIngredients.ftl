    ...
    <div id="product-info" class="mb clearfix">
        <div id="product-info-quick">
            <img src="<#if (product.label?? && product.label.photo?exists && product.label.photo?length > 0)>${cp}files/${product.label.photo}<#else/>${cp}images/product.png</#if>"
                 width="217" height="217" alt="<#if product.name??>${product.name}<#else/>Product</#if> label"
                 class="picture"/>
        </div>

        <div id="product-info-details">
            <div id="product-info-details-photo" class="picture-big overflow-drag-widget">
                <img src="<#if (product.ingredients?? && product.ingredients.photo?exists && product.ingredients.photo?length > 0)>${cp}files/${product.ingredients.photo}<#else/>${cp}images/product.png</#if>"
                     alt="<#if product.name??>${product.name}<#else/>Product</#if> ingredients"/>
            </div>
        ...
        <div class="field clearfix">
            ...
            <div class="input clearfix">
                ...
                <#if product.conservants?? && (product.conservants?size >0)>
                    <p><@spring.message code="createproduct.form.additives.recognized"/>
                        : <#list product.conservants as e>${e.number} </#list></p>
                </#if>
            </div>
        </div>
    <#--
