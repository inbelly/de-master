...
<div id="product" class="inline">
    <div id="product-info" class="clearfix">
        ...
        <#if !product.approved>
            <img src="${cp}img/aproval_1.png" alt="not approved" style="position: absolute;"
                 title="<@spring.message code="productList.productNotApproved" />"/>
        </#if>
            <img src="<#if (product.label?? && product.label.photo?exists && product.label.photo?length > 0)>${cp}files/${product.label.photo}<#else/>${cp}images/product.png</#if>"
                 width="217" height="217" alt="${product.name?xhtml} photo" title="${product.name?xhtml}"
                 class="picture"/>
        </div>
        <div id="product-info-details">
            <div id="product-info-details-photo" class="picture-big overflow-drag-widget">
                <img src="<@getPhotoLink product.ingredients />" width="1024" height="768"
                     alt="${product.name?xhtml} ingredienser" title="${product.name?xhtml} ingredients"
                     onClick="window.open('<@getPhotoLink product.ingredients />', 'WindowC', 'width=${product.ingredients.width}, height=${product.ingredients.height}, scrollbars=yes');"/>
            </div>
...
<#-- if (currentUser?? && currentUser.admin) || !product.confirmed && (!currentUser?? ||  (currentUser?? && (product.canBeConfirmedBy(currentUser) || product.canBeEditedBy(currentUser)))) -->
    <div id="waiting-approval" class="mb clearfix">
        <div class="message">
            <p>
            <#if !product.confirmed && !currentUser?? || product.canBeConfirmedBy(currentUser)><@spring.message code="product.isWaitingForApproval" /></#if> <@spring.message code="product.checkDetailsBelow" />
            <#if !currentUser?? || product.canBeEditedBy(currentUser)><@spring.message code="product.youCan" /> <a href="product/edit?id=${product.id}"><@spring.message code="product.edit"/></a>
                <@spring.message code="product.ifItIsNotCorrect" /></#if>
            </p>
...
<#if (!currentUser?? ||  product.canBeReportedBy(currentUser)) >
    <div class="message">
        <p<@spring.message code="product.youCan" /> <a href="product/report?id=${product.id}"><@spring.message code="product.report" /></a> <@spring.message code="product.ifItIsNotCorrect" /></p>
    </div>
    <br/>
</#if>
    ...
    <div id="product-info-edit" class="mb clearfix">
        <div class="left" class="clearfix">
            <div class="field">
                ...
                <div class="text">
                    <h4 id="company"><a
                            href="${cp}spring/productList/filterByCompany?company=${product.company?url('utf-8')}"><#if product.company??>${product.company?html}</#if></a>
                    </h4>
                </div>
            </div>
            <div class="field">
            ...
            <div class="field">
                ...
                <div class="text">
                    <h4 id="category"><a
                            href="${cp}spring/productList/filterByCategory?category=${product.category.id?url('utf-8')}"><#if product.category??>${product.category.name?html}</#if></a>
                    </h4>
                </div>
            </div>
            <div class="field">
        ...
        <#include "_productHistory.ftl" />
            ...
            <div class="fb-like"
                 data-href="http://<@spring.message code="site.domain" />${cp}spring/product?id=${product.id}"
...
<#else/>
    ...
    <div class="mb fr clearfix">
        <div class="addthis_toolbox addthis_default_style">
            <a href="http://addthis.com/bookmark.php?v=250&amp;username=xa-4bfe640360a8a9a8"
               class="js addthis_button_compact">Share</a>
        ...
        <script type="text/javascript"
                src="http://s7.addthis.com/js/250/addthis_widget.js#username=xa-4bfe640360a8a9a8"></script>
    </div>
